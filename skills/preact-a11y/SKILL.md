---
name: preact-a11y
description: Use when designing, implementing, testing, or reviewing accessibility in Preact apps — semantic HTML, ARIA, keyboard flow, focus management, forms, contrast, motion, responsive behavior, or WCAG 2.2 concerns.
---

# Preact Accessibility

Use this skill for accessibility work in Preact frontend applications.
...
- Use `useRef` + `.focus()` for programmatic focus management. Preact supports the same hooks API as React.
- Use signals (`@preact/signals`) for reactive state — bind ARIA attributes directly from signals.
...
- Use `componentDidCatch` in class components for error boundaries.
- Pass `aria-*` props directly to native elements — Preact supports them natively.
